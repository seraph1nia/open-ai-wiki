...
Official sources, search, GitHub, articles, video
      ...
      deduplication + structured extraction
       ...
       auditable ingestion/inbox candidate
                    │
                    ▼
       controlled OKF/OpenWiki synthesis
                    │
                    ▼
                openwiki/
                 ├─ Astro → GitHub Pages
...
All model-backed stages use OpenRouter's current `~deepseek/deepseek-v4-flash-latest` alias by default, including discovery, ranking, extraction, and synthesis. Override `OPENROUTER_MODEL` if you want to pin a dated model. `OPENROUTER_BASE_URL`, `OPENROUTER_APP_NAME`, and `OPENROUTER_HTTP_REFERER` are optional. Tests inject mocked Responses API results and make no paid calls.
...
State format version 1 records the last successful harvest, canonical URLs, content hashes, publication dates, decisions, and scores. Its versioned boundary is intentionally simple to migrate to SQLite later.
...
Add `-- --dry-run` to manual ingestion or harvesting to print scores and proposed wiki changes without changing `openwiki/`, the inbox, or persistent state:
...
`ingestion/src/openwiki.ts` isolates synthesis behind `synthesizeCandidate(candidate)`. It asks the model to inspect the existing wiki outline, choose enrichment before duplication, preserve existing knowledge, and make one controlled change. Source provenance is always a separate `type: Source` object.

The automated pipeline does not require or spawn the OpenWiki CLI: Astro reads the resulting `openwiki/` tree directly, and the repository's synthesis layer uses the shared OpenRouter client. To run LangChain's official OpenWiki UI/workflows as an optional companion, install it with pnpm and initialize its personal mode:
...
OpenWiki's onboarding stores its own provider configuration in `~/.openwiki/.env`. Select OpenRouter and `~deepseek/deepseek-v4-flash-latest` there if you want the companion CLI to use the same model. Its personal wiki lives separately under `~/.openwiki/wiki/`; configure a local `git-repo` connector for this repository when you want that personal wiki to ingest this knowledge base. Running `openwiki code --update` from the repository is optional and is a separate writer to `openwiki/`, so review its changes just like an automated harvest.
...
Astro's content layer loads `../../openwiki/**/*.md` directly. Build-time utilities create collection routes, internal relationships, automatic backlinks, and a lightweight client-side search index over titles, descriptions, bodies, tags, and types. No server or API key exists in the deployed site.
...
`.github/workflows/harvest.yml` runs daily at 06:00 UTC and can be dispatched manually. It installs the exact mise toolchain, restores the pnpm store, performs the harvest, validates the repository, and opens a review pull request with the generated human-readable summary. It never auto-merges.
...
In repository settings, select **GitHub Actions** as the Pages source. `.github/workflows/pages.yml` typechecks, tests, builds, uploads the static artifact, and deploys after every push to `main`. Astro derives a project-repository base path from `GITHUB_REPOSITORY`, so assets and navigation work at `https://USERNAME.github.io/REPOSITORY/`.
...
Strict TypeScript, runtime validation, linting, formatting, deterministic unit tests, a frozen lockfile in CI, bounded acquisition, content hashes, canonical URLs, and an auditable candidate boundary protect the pipeline. Acquired content is treated as untrusted evidence. Secrets belong only in environment variables or GitHub Actions secrets and must never use a public Astro environment prefix.
