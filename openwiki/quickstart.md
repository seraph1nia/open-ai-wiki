...
- **AG-UI** is event-based (≈16 event types), has official TypeScript/Python plus Java and Go SDKs (SSE-based) and a community Swift SDK, and lists **human-in-the-loop** as a first-class stack feature. Open proposals would bridge Oracle's Open Agent Spec and Microsoft's agent-governance toolkit onto AG-UI (watchlist).
