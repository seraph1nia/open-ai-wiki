...
Blog and article feeds are ingested once per post. Before using any post from a followed feed, consult the append-only [blog post ingestion ledger](sources/blog-post-ledger.md) and skip every post already listed there — a listed post is closed and must not be refetched, re-summarized, or re-adopted. After a run, append one row per post considered, including posts that yielded nothing durable, and never delete or rewrite existing rows.
