...
- **Surface** — a distinct, controllable region of the client's UI identified by `surfaceId` (main content area, side panel, chat bubble); one agent stream can manage multiple surfaces independently.
- **Component** — a UI element (Button, TextField, Card, Row, Column, …) expressed as a *typed abstract node*.
- **Data Model** — application state that components bind to (data binding).
- **Catalog** — the available component types, defined in a **Catalog Definition Document** (a JSON Schema document) so clients and servers can negotiate which catalog to use. `catalogId` is used by A2UI SDKs and catalog negotiation (set to the same URI as JSON Schema `$id`).
- **Message** — a JSON object such as `surfaceUpdate`, `dataModelUpdate`, or `beginRendering`.
...
A2UI is designed to interop with the rest of the agent-UI space rather than replace it. The site documents explicit cross-integrations: **A2UI over MCP**, **MCP Apps in A2UI**, and **A2UI in MCP Apps** (see the [MCP Apps page](/protocols/mcp-apps.md)). Its roadmap mentions supporting more renderers (React, Jetpack Compose, SwiftUI) and more transports (REST).

## Status

- **Confidence:** source-backed (a2ui.org specification v1.0/v0.8, data-flow, who-is-it-for, and roadmap pages, plus the `a2ui-project/a2ui` repo README; single primary source, not independently cross-checked).
- Actively developed and shaped by community roadmap feedback; current stable is v0.9.1, v1.0 still a candidate.
